...
def test_volume_24h_above_threshold_passes():
    ...
    result = risk.evaluate_entry_quality(snap, {}, signal_score=80,
                                          analysis_verdict="✅ 看起来健康")
    assert result["tier"] in ("full", "half"), f"应通过，实际 skip: {result['hard_block']}"
    assert not any("24h 成交额" in x for x in result["hard_block"])
    print(f"OK 24h 成交额充足通过，tier={result['tier']}")


def test_volume_24h_at_threshold_blocked():
    snap = _base_snap()
    snap["volume_24h_usd"] = 5000
    result = risk.evaluate_entry_quality(snap, {}, signal_score=80,
                                          analysis_verdict="✅ 看起来健康")
    assert result["tier"] == "skip"
    assert any("24h 成交额" in x for x in result["hard_block"])
    print(f"OK 24h 成交额边界值被拒: {result['hard_block'][0]}")


def test_volume_24h_missing_blocked():
    snap = _base_snap()
    snap["volume_24h_usd"] = None
    result = risk.evaluate_entry_quality(snap, {}, signal_score=80,
                                          analysis_verdict="✅ 看起来健康")
    assert result["tier"] == "skip"
    assert any("成交额缺失" in x for x in result["hard_block"])
    print(f"OK 24h 成交额缺失被拒: {result['hard_block'][0]}")
...
if __name__ == "__main__":
    tests = [
        ...
        test_volume_24h_above_threshold_passes,
        test_volume_24h_at_threshold_blocked,
        test_volume_24h_missing_blocked,
    ]
    failed = 0
    for t in tests:
        try:
