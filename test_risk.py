...
def test_entry_quality_overheated_block():
    snap = {"change_15m_pct": 10, "volume_24h_usd": 6000}  # 不重要，verdict 过热直接拒
    result = risk.evaluate_entry_quality(snap, {}, signal_score=80,
...
def test_entry_quality_low_volume_block():
    ...
    result = risk.evaluate_entry_quality(snap, {}, signal_score=80,
                                          analysis_verdict="✅ 看起来健康")
    assert result["tier"] == "skip", f"24h 成交额不足应 skip，实际 {result['tier']}"
    assert any("24h 成交额" in x for x in result["hard_block"])
    print(f"OK 低成交额硬否决: {result['hard_block'][0]}")
